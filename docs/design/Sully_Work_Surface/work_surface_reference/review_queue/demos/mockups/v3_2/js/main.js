/* ==========================================================================
   SULLY WORK SURFACE MOTION MOCKUP ENGINE - v3.2 SEMANTIC MOTION
   ========================================================================== */
...
<defs>
  ...
  <!-- Worker: DeepSeek -->
  <g id="icon-deepseek"><path fill="currentColor" d="M22 6.5c-.2-.1-.3.1-.4.2 0 .1 0 .1-.1.2-.3.3-.6.5-1.1.5-.7-.1-1.3.1-1.8.7-.1-.7-.5-1.1-1.1-1.4-.3-.1-.6-.2-.8-.5-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6L12 18c-1.7-1.3-2.5-1.8-2.9-1.8-.3 0-.3.4-.2.6v.7c.1.2.2.5-.1.7-.5.3-1.5-.1-1.5-.1-1.1-.7-2-1.5-2.6-2.7-.6-1.2-1-2.4-1.1-3.7 0-.3.1-.4.4-.5.4-.1.8-.1 1.2-.1 1.7.3 3.1 1.1 4.3 2.3.7.7 1.2 1.5 1.8 2.3.6.9 1.2 1.7 2 2.4.3.2.5.4.7.6-.6.1-1.6.1-2.3-.7z"/></g>
  <!-- System: Memory -->
...
function logTelemetry(msg) {
  ...
  if (logBox) {
    ...
    logSpan.innerText = `\n[V3.2 INT:${currentIntensity.toUpperCase()}] ${msg}`;
...
function renderGraph(svgId, preset) {
  ...
  workers.forEach((w, idx) => {
    ...
    // 2. Draw packets based on motionType
    // Only researching, building, verifying have packets. Complete, blocked, waiting do not.
    if (w.motionType === 'researching' || w.motionType === 'building' || w.motionType === 'verifying') {
      const numPackets = (w.motionType === 'verifying') ? 1 : 3; // Verifying might look cleaner with 1 oscillating, building with 3. Let's do 3 for all, staggered.
      for (let i = 0; i < 3; i++) {
        // If verifying, maybe just 2 packets overlapping looks like steady checking
        if (w.motionType === 'verifying' && i === 2) continue;
        ...
        if (w.motionType === 'researching') {
          ...
          packetUse.style.animationDelay = `${i * 2.25}s`;
        }
  ...
  workers.forEach((w, idx) => {
    ...
    // Stagger breathing delays slightly
  ...
  // Base task state on preset activeMotionType
