...
- ✅ **Phase B (one morphing card) — SHIPPED + browser-verified** (same commits). **A+B MERGED to `main` (PR #5) + deployed live on `:18769` 2026-06-03.**
- ✅ **Phase C (ask-before-dispatch tap-to-confirm buttons) — SHIPPED on branch `feat/ask-before-dispatch-buttons`** (commit `51f7f0e`, browser-verified). Ask message posts `status='pending_approval'`; MessageFeed renders Run/Not-now; page handler POSTs `/api/chat/dispatch/confirm` + re-polls. Endpoint: `getProposalByTaskId` returns only a still-`gated` row → double-tap / post-expiry taps clear the buttons (`resolveProposalMessage`) instead of re-dispatching; tailnet guard. Typed "yes" still works. 214 tests, 0 type errors. Browser-verified: buttons render, dismiss round-trips (proposal aborted, buttons cleared, hold-off posted). Awaiting operator phone-QA + merge.
