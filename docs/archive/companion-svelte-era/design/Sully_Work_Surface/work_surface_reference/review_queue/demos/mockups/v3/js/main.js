/* ==========================================================================
   SULLY WORK SURFACE MOTION MOCKUP ENGINE - ACTIVE OWNERSHIP v3
   ========================================================================== */
...
<defs>
  ...
  <!-- Worker: DeepSeek -->
  <g id="icon-deepseek">
    <path fill="currentColor" d="M22 6.5c-.2-.1-.3.1-.4.2 0 .1 0 .1-.1.2-.3.3-.6.5-1.1.5-.7-.1-1.3.1-1.8.7-.1-.7-.5-1.1-1.1-1.4-.3-.1-.6-.2-.8-.5-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6-.1-.2-.2-.4-.3-.6L12 18c-1.7-1.3-2.5-1.8-2.9-1.8-.3 0-.3.4-.2.6v.7c.1.2.2.5-.1.7-.5.3-1.5-.1-1.5-.1-1.1-.7-2-1.5-2.6-2.7-.6-1.2-1-2.4-1.1-3.7 0-.3.1-.4.4-.5.4-.1.8-.1 1.2-.1 1.7.3 3.1 1.1 4.3 2.3.7.7 1.2 1.5 1.8 2.3.6.9 1.2 1.7 2 2.4.3.2.5.4.7.6-.6.1-1.6.1-2.3-.7z"/>
  </g>
...
// 2. SIMULATION PRESETS CONFIGURATIONS (V3 ACTIVE OWNERSHIP REFINEMENTS)
const PRESETS = {
  'cc-only': {
    ...
    activeClass: "working", // working, checking, blocked, complete
    workers: [
      { key: "CC", icon: "icon-dispatch", role: "Coordinator", state: "active", desc: "Researching index tables" }
    ],
    phases: [
  ...
  'cc-verify': {
    ...
    workers: [
      ...
      { key: "Ver", icon: "icon-verify", role: "QA Verify", state: "active-checking", desc: "Running TLS v1.3 scans" }
    ],
    phases: [
  ...
  'agy-verify': {
    ...
    workers: [
      ...
      { key: "AGY", icon: "icon-antigravity", role: "Antigravity Agent", state: "active", desc: "Writing drag handlers" },
  ...
  'dpsk-verify': {
    ...
    activeClass: "checking",
    workers: [
      { key: "CC", icon: "icon-dispatch", role: "Coordinator", state: "complete", desc: "Routing complete" },
      { key: "DPSK", icon: "icon-deepseek", role: "DeepSeek", state: "active-checking", desc: "Verifying trigger constraints" },
      { key: "Ver", icon: "icon-verify", role: "QA Verify", state: "waiting", desc: "Awaiting constraints" }
    ],
    phases: [
  ...
  'multi-worker': {
    ...
    workers: [
      ...
      { key: "AGY", icon: "icon-antigravity", role: "Antigravity Agent", state: "active", desc: "Synthesizing similarity matrices" },
  ...
  'blocked': {
    ...
    activeClass: "blocked",
    workers: [
      { key: "CC", icon: "icon-dispatch", role: "Coordinator", state: "active-blocked", desc: "Halted at Gate" },
...
// GRAPH DRAWING LOGIC WITH OWNERSHIP HIGHLIGHTS
function renderGraph(svgId, preset) {
  ...
  // 1. Draw paths (Edges) with active ownership highlights
  workers.forEach((w, idx) => {
    ...
    const isActiveWorkerRoute = (w.state === 'active' || w.state === 'active-checking' || w.state === 'active-blocked');

    const path = document.createElementNS("http://www.w3.org/2000/svg", "path");
    path.setAttribute("d", pathD);
    
    // Highlight route only for active worker (v3 improvement)
    ...
    // 2. Draw glide packets ONLY on active route paths (v3 improvement)
    const togglePackets = document.getElementById('toggleSpeed');
    const packetsEnabled = togglePackets ? togglePackets.checked : true;

    if (packetsEnabled && isActiveWorkerRoute && preset.status !== 'complete' && preset.status !== 'blocked') {
      for (let i = 0; i < 3; i++) {
  ...
  // 3. Draw Worker Nodes with Breathing Focus highlights
  workers.forEach((w, idx) => {
    const pos = layout[idx];
    const g = document.createElementNS("http://www.w3.org/2000/svg", "g");
    g.setAttribute("class", `node-group-animated stagger-${(idx % 4) + 1}`);

    const isActiveWorker = (w.state === 'active' || w.state === 'active-checking' || w.state === 'active-blocked');

    // Pulse Ring: strong pulse for active, quiet static circles for others
...
function updateUI() {
  ...
  // Active Ownership Glanceable Label (v3 improvement)
  ...
  // Worker registry with row highlights (v3 improvement)
  const workersGrid = document.getElementById('workersGrid');
  workersGrid.innerHTML = data.workers.map(w => {
    const isActive = (w.state === 'active' || w.state === 'active-checking' || w.state === 'active-blocked');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('toggleSpeed').addEventListener('change', (e) => {
    logTelemetry(`Packet flows ${e.target.checked ? 'Enabled' : 'Disabled'}`);
    updateUI();
  });
