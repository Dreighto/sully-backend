...
Instead of general glowing lines, the animations are designed to explain to Captain and CC exactly *how* work is flowing through the system.
...
3. **Verifying (Inspection)**:
   ...
   - **Precision Sync**: The verifier node fires high-frequency scan pulses *only* during the active orbit, and the `TASK` node ripples precisely when the packet returns back.
