/* ==========================================================================
   SULLY WORK SURFACE - REAL ASSETS V4 ENGINE
   ========================================================================== */
...
const PRESETS = {
	...
	'dpsk-verifying': {
		...
		proof: {
			verdict: 'pending',
			score: null,
			checks: [{ name: 'Schema check', status: 'pending', detail: '' }]
		}
	},
	'gmi-brainstorming': {
		...
		workers: [
			{
				...
				role: 'Research',
				motionType: 'researching',
				desc: 'Synthesizing log similarities'
			}
		],
		phases: [
	...
	'cdx-reviewing': {
		...
		proof: {
			verdict: 'go',
			score: 94,
			checks: [{ name: 'Structure audit', status: 'pass', detail: 'Routes checked' }]
		}
	},
	'multi-worker': {
		...
		systemStatus:
			'Antigravity compiling drag code, Claude researching schemas, Codex checking files.',
		state: 'Working',
		status: 'working',
	...
	complete: {
		...
		proof: {
			verdict: 'go',
			score: 100,
			checks: [{ name: 'Deployment checks', status: 'pass', detail: 'All tests pass' }]
		}
	},
	stopped: {
...
function renderGraph(svgId, preset) {
	...
	const activeWorkers = preset.workers.filter(
		(w) => w.motionType !== 'complete' && w.motionType !== 'waiting'
	);
	...
	// 1. Concentric background field rings (core field behind TASK)
	const coreFieldG = document.createElementNS('http://www.w3.org/2000/svg', 'g');
	coreFieldG.setAttribute('class', 'core-field');
	coreFieldG.setAttribute('opacity', '0.15');

	const ringRadii = [35, 55, 75];
	ringRadii.forEach((r) => {
		const ring = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
		ring.setAttribute('cx', targetX);
		ring.setAttribute('cy', targetY);
		ring.setAttribute('r', r);
		ring.setAttribute('stroke', 'var(--sully-text-muted)');
		ring.setAttribute('stroke-width', '0.75');
		ring.setAttribute('stroke-dasharray', r === 35 ? '1 4' : r === 55 ? '2 6' : '3 8');
		ring.setAttribute('fill', 'none');
		coreFieldG.appendChild(ring);
	});
	svg.appendChild(coreFieldG);
	...
	routes.forEach((route) => {
		...
		// 2. Dual-line "lane" rendering for active primary routes
		if (route.isPrimary && !route.isEdge) {
			const backingPath = document.createElementNS('http://www.w3.org/2000/svg', 'path');
			backingPath.setAttribute('d', pathD);
			backingPath.setAttribute(
				'class',
				`edge-line-backing route-${route.motionType}-backing ${priorityClass}`
			);
			svg.appendChild(backingPath);
		}

		// Static Base Edge Path (Inner track)
		const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
		path.setAttribute('d', pathD);
		path.setAttribute('class', `edge-line route-${route.motionType} ${priorityClass}`);
		svg.appendChild(path);

		// Scanner Sweep overlay path
		if (
			route.motionType === 'researching' ||
			route.motionType === 'building' ||
			route.motionType === 'verifying'
		) {
			...
			sweepPath.setAttribute(
				'class',
				`edge-sweep-line route-${route.motionType}-sweep ${priorityClass}`
			);
			svg.appendChild(sweepPath);
		}

		// Draw Semantic Payload Packets along active routes
		if (
			route.motionType === 'researching' ||
			route.motionType === 'building' ||
			route.motionType === 'verifying'
	...
	activeWorkers.forEach((w, idx) => {
		...
		circle.setAttribute(
			'class',
			`node-circle node-${w.motionType} ${breathDelayClass} ${priorityClass}`
		);
	...
	routes.forEach((route) => {
		...
		circle.setAttribute(
			'class',
			`node-circle node-${route.motionType} breath-delay-1 ${priorityClass}`
		);
		g.appendChild(circle);
	...
	// Central Task landing handoff ripple
...
function updateUI() {
	...
	const renderTimeline = (containerId) => {
		...
		if (!container) return;
		container.innerHTML = data.stageProgress
			.map((s) => {
				let icon = '';
	...
	if (data.proof && data.proof.checks && data.proof.checks.length > 0) {
		checksHtml =
			...
			data.proof.checks
				.map(
					(c) =>
						`<li><span class="proof-check-status ${c.status}">${c.status}</span> ${c.name} <span class="proof-check-detail">${c.detail || ''}</span></li>`
				)
				.join('') +
			'</ul>';
	}
...
document.addEventListener('DOMContentLoaded', () => {
	...
	presetBtns.forEach((btn) => {
		btn.addEventListener('click', () => {
			...
			// Polish: Trigger wake animation on preset change
			const card = document.getElementById('sullyCard');
			card.classList.remove('wake-active');
	...
	layoutBtns.forEach((btn) => {
		btn.addEventListener('click', () => {
			...
			if (oldState !== currentState) {
				// Trigger wake ripple on layout transition
				const card = document.getElementById('sullyCard');
				card.classList.remove('wake-active');
				void card.offsetWidth;
				card.classList.add('wake-active');
